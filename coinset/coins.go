...
func (cs *CoinSet) TotalValue() (value luxutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue luxutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue luxutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount luxutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue luxutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue luxutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue luxutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue luxutil.Amount, coins []Coin) (Coins, error) {
...
// luxutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() luxutil.Amount {
	return luxutil.Amount(c.txOut().Value)
}
