...
import (
	...
	"github.com/bitbandi/luxutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := luxutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(luxutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := luxutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := luxutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := luxutil.NewBlockFromBytes(blockBytes)
	if err != nil {
